...
/*
    2. interface
     ...
     - 인터페이스와 타입별칭의 동작방식은 유사하나
      각기 존재하는 기능이 다름.
*/
interface Animal { // ; , (공백) 으로 해놔도 됨
    name : string; // -> string;에서 ;여기를 말하는 거임
    kind : string;
    birth? : Date;
}
type AnimalAliases = {
...
// a&b
const pizza:Food = {name:'파파존스',flavor:"파인에플 피자...."}
// a
const bugger:Food = {name:'맥도날드'};
//b
const flavor:Food = {flavor:'단짠맛'};

type _Food = Ingredient & Taste;
// {name:string, flavor:string}

//type Sample = string & number; // 쓸수는 있지만 안쓰는것을 추천
...
export type Response = SuccessResponce | FailResponse;
type str = string;
type bool = boolean;

/*
    5. 인덱스 시그니쳐
     - 객체의 속성명에 대햐여 명확히 인지하지 못하는 경우
     사용하는 문법
...
export default animal1; // 이거 항상 추가 하기