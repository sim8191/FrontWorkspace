...
// 실습문제 1) 다음 조건을 만족하는 Object Type Aliases 만들기
// typescript
// type Academy = {
//     ??
// };
...
/* Academy에는 `name`속성이 **필수**로 들어가며 **항상** 'kh'값만 들어가며 수정될 수 없습니다.
* Academy에는 `class`속성이 **필수**로 들어가며 항상 **문자열로된 배열데이터**만 들어갑니다.
* Academy에는 `location`속성이 들어갈 수도 있으며 **문자열**만 들어갑니다.
* Academy에는 `employee`속성이 들어갈 수도 있으며 다음과 같은 데이터를 가질 수 있습니다
...
// 1. type1과 type2를 관리할 수 있는 타입별칭 만들기
// 2. type3과 type4를 관리할 수 있는 타입별칭 만들기
// 3. MBTIType1과 MBTIType2를 이용하여 type5를 관리할 수 있는 타입별칭 만들기
// typescript
// type MBTIType1;
...
type MBTIType3 = MBTIType1 & MBTIType2;// 교집합?
const type5:MBTIType3 = {mbti : 'ISTP' , bestGunghab : ['ESFJ', 'ESTJ'], feature : ['망상가', '공감능력 부족']  }; 
// type6는 컴파일 에러 발생
// const type6:MBTIType3 = {mbti : 'ISTP' , bestGunghab : ['ESFJ', 'ESTJ'] , worstGunghab : 'ENFJ'}; 


// 실습문제 3) 다음 조건을 만족하는 인터페이스들을 만드시오.
...
// 실습문제 4) 다음 조건을 만족하는 Object Type Aliases을 정의하시오

/* ObjType의 name에는 **항상** 'mkm'만 들어갈 수 있습니다.
* ObjType의 printName은 반환 값이 없는 메서드입니다.
* ObjType의 printName은 매개변수로 **함수**를 전달받습니다.
...
/* Coin에는 반드시 **ticker**와 **market**속성이 존재해야 합니다.
...
/* 매개변수로 전달받은 user객체에서 **name**과 **age**값만 뽑아서 첫 번째 console.log를 작성하고,
* user객체의 나머지 정보를 뽑아 **others**로 만든 후 두 번째 console.log에 대입하세요.
* ??공간에서만 코드를 작성해야 합니다. */

// typescript
// function greetUser(??) {
//     console.log(??); // 안녕하세요. 제이름은 홍길동 30살 입니다.
//     console.log(??); // 추가정보 : { email: 'hong@test.com', location: '서울' }
// }
// const user = {
//     name: '홍길동',
//     age: 30,
...
export default Academy; // 이거 항상 추가 하기