...
function BoardRegister({list, setList}: {list: typeof boardList, setList: React.Dispatch<React.SetStateAction<typeof boardList>>}) {
  ...
  const onRegisterHandler  = () => {
    const { boardTitle, boardWriter, boardContent} = boardInput; 
    if (!boardTitle || !boardWriter || !boardContent) {
    alert("모든 항목을 입력해주세요!");
    return;
  }
  const lastNo = list.length > 0 ? Math.max(...list.map(item => item.boardNo)) : 0;
  ...
  const newBoard = {
    ...
    boardWriter,
    boardContent,
    boardDate: formatDate(new Date()) // 현재 시간 포맷팅
  };
  setList([...list, newBoard]);
