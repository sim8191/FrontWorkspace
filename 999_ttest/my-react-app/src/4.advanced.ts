import animal from "./1basic-test";
...
const result = returnGeneric<string>("hello"); // 자동적으로 타입추론을 함
...
function config<T extends Options = {theme:'dark'}>(options?:T):T{
    ...
    // 널병합연산자 ??
    // - a ?? b -> a가 null또는 undefind가 아니라면 A,
    //   맞다면 B반환
    return options ?? {theme:"dark"} as T;// 3개중 이걸 많이 쓰게 될지도?
}
console.log("========");
...
/*
    4. Utility Type
     ...
     1) Partion<T>
      ...
      - 모든 속성을 필수, 읽기전용으로 만들어주는 Required<T>,
        Readonly<T>도 존재한다.
*/
interface User {
    id:number;
...
const PartionUser:Partial<User> = {id:1, name:"aaa"};
...
// {name:string, email:string} // 이런식으로 2개만 선택해서 타입을 만드는거임
console.log("========");
/*
    3) Omit<T, K>
     - 객체 타입에서 특정 속성을 제외한 나머지만 남긴 채
     타입을 만들때 사용
*/
