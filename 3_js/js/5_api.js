test1.onclick = function(){
    ...
    console.log(str.substring(2,4)); // 2,3번이덱스값 추출
...
test4.onclick = function(){
    ...
    console.log(7 % Number('4')); // 3 // 이것도 그냥 쓰면 되는 대 자동으로 형변환 된다는 것보여주기 위해 이렇게 사용한거임
    // 즉, +을 제외환 산술연산시 문자열은 number자료형으로
    // 자동형변환된다.

    console.log(7 % 'a');// 숫자형태가 아닌 문자열을 숫자로 형변환시
    // 반환된 값 (NaN). NaN과 기타 값들간의 연산결과는 항상 NaN이 반환
    // NaN(number자료형임)
    ...
    // 모든 연산시에는 원시값과 원시값끼리만 연산이가능하기 대문에, 참조변수는 원시값으로
    // 변환된 후 연산처리 된다.
    // [] -> [].toString -> '' + 1 => '1'
...
test5.onclick = function(){
    ...
    // Number는 정수/실수로 반환이 가능. 단, 변환불가 문자가 있는 경우 NaN을 반환
...
test7.onclick = function(){
    ...
    */
    ...
    console.log("0" == 0); // true("0"기에 number를 씨워서 형변환을 시킴 그래서 true임)
    console.log([] == 0); // true([]가 0으로 형변환 되서 true임)
    console.log([] == "0"); // false([]이 빈문자열이기 때문에 false임)
    // 위에것들을 잘 확인해볼것
};
...
test8.onclick = function(){
    /*
        일반 동등비교 연산자
         == , !=
         왼쪽 오른쪽 타입이 서로 다르다면 "자동현변환"후 비교 수행

        엄격 동등비교 연산자
         ===, !==
         왼쪽 오른쪽 타입이 서로다르다면 바로 false를 반환하는 연산자()
