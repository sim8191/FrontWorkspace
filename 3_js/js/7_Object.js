test1.onclick = function () {
    /*
        Object
         ...
         - 속성병 : 식별자의 역할을 하며 보통 문자열로 작성
         - 속성값 : 모든타입의 값을 저장할 수 있다.
    */
    var obj1 = {
        // "name" : '홍길동', // 보통 ""를 생략해서 씀
        name: '홍길동',
        "age": 33,
        married: true,
        hobby: ['축구', '야구'],
...
test3.onclick = function () {
    game = {
        ...
        shutdown: function () {//test4를 위해 만든거
            console.log(this.title + "을 종료합니다..")
            this['end-time'] = new Date();
        },
        duration: function () {//test4를 위해 만든거
            // 두 시간의 차이를 ms로 반환
...
test4.onclick = function () {
    // game.shutdown(); 단순하게 이렇게만 써도 쓸수는 있으나 위험한 코드임 
    // game.shutdown(); 이렇게 쓰면 무조건 test3을 실행시킨후 4를 실행시켜야지만 오류발생이 안됨
    game && game.shutdown();
    game && game.duration();
}
    ...
    /*
        생성자 함수
         ...
         - 클래스를 정의하기 위한 용도의 함수로, 생성자 함수로 만든 함수는
           new 연산자와 함께 호출해야 한다.
         - 생성자 ㅎ마수느 첫 글자를 클래스와 같이 대문자로 작성한는 것이 관례
         ...
         * prototype?
           - 모든 함수가 묵시적으로 가지고 있는 속성으로, 부모객체를 저장해두는
             저장공간.
    */
    function Pet(kind, name, breed, weight) {//대문자로 시작하면 class함수다라고 암묵적으로 표현한거
        // {}
        // 필드부
...
test8.onclick = function () {
    ...
    */
    ...
    class Pet extends Animal {
        #hobby; // #은 외부에서 간섭못하게 막는 역과 privait같은 느낌
    ...
    // console.log(cat.hobby); // 이렇게 쓰는 건 의미 없음
    // 왜부에서 막 함부로 쓸수 있으니위에서 #hobby 이렇게 쓰면 막힘
