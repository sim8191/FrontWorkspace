...
*/
// querySelector("#id")// id값이 id인 값을 찾겠다. 없다면 null
document.querySelector("#id").onclick = function(){
    ...
    // 3. id속성값으로 접근하기.
    // - 브라우저는(몇몇 브라우저 제외) id값은 요소를 전역변수형태로 등록
    //   하기 때문에 id속성값을 통해 욧를 바로 획득할 수 있다.
...
document.querySelector("#class-name").onclick = function(){
    // 클래스명으로 요소를 선택하는 방법들
    var groups = document.getElementsByClassName("group"); // 조금 오래된 방식
    groups = document.querySelectorAll(".group"); // 앞으로 이걸 많이 쓸 예정

    // 버튼을 누를때 마다 선택한 요소의 배경을 바꾸기
...
document.querySelector("#name-selector").onclick = function(){
    ...
    // input[name=hobby]는 hobby가 들어견 경우 [name=hobby] 이름이 hobby인 경우 다가져오기
    console.log(hobbies);
...
document.querySelector("#email-confirm").onclick = function(){
    // var chr = document.getElementsByClassName("userEmail"); 
