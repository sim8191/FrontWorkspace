'use client' //페이지 컴포넌트를 클라우드로
// 그이유는 서버 컴포넌트로 하면 next.js로 넘어가서 spring에서 못함
// 허나 원래는 서버컴포넌트로 하는게 정사임 지금은 급해서 이렇게 하는거라고 함
        ...
        .then(response => {
            ...
            /*
                ...
                #2 JWT저장위치
                 ...
                 2) sessionStorage
                  - 탭단위 세션 저장소로 탭을 닫으면 데이터도 삭제된다. (데이터유지시간이 짧다.)
                  - xss공격에 취약하다.
                 
                 3) 쿠키
                  - 쿠키는 특정 도메인의 경로에 지정된 시간동안 저장되는 데이터
                  - 쿠키는 HTTP통신시 항상 자동으로 전될된다.
                  - 기본쿠키는 JS로 접근하여 탈취될 수 있으나 http-only설정시 js로 쿠키제어가 불가능.
                  (xss에서 안전)

                 4) 리액트메모리
                  - 리덕스에 보관시 메모리에만 저장되며, 새로고침시 데이터가 소멸한다.
                  - xss공격의 위험이 존해하긴 하나 유지시간이 굉장히 짧기 때문에, 공격을 최소화 할
                  수 있다.
            ...
            */
            ...
            if(err.response?.status === 404){
                const doSigUp = confirm("등록된 계정이 없습니다. 회원가입을하시겠습니까?")
                if(!doSigUp){
