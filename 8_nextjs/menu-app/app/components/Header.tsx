...
export default function Header(){
    ...
    // if(!mounted){ // 이게 있어야 새로고침 오류가 없어짐 근디 아래에 새로 만듬
                            ...
                            /*
                                #1. 조건부 렌더링 사용시 하이드레이션 에러 발생
                                 - 서버컴포넌트의 상태와 하이드레이션시 결과가 다른 경우 발생하는 에러
                                    - > isAutenticated는 redux에 존재하는 데이터인데, 
                                    이 코드는 클라이언트 컴포넌트에서만 알 수 있기 때문에
                                 - 하이드레이션에러를 방지하기 위해선, 서버, 클라이언트 모두 동ㅇ칠한
                                    값을 보게 해야 한다. => mounted설정.
                            */
                            !mounted ? (<li>Loading...</li>) : // 이게 있어야 새로고침 오류가 없어짐(이거는 극소범위만 적용 시킬려고) 이것을 하는이유
                            ...
                            (
                               <>
                                <li className="nav-item" >
                                    {/* 
                                        #2. Image
                                         - mext.js에서 img대신 사용하는 Image최적화 태그
                                         - 이미지가 실제 화면에 나타날때까지 로딩을 지연하는 지연로딩기능
                                         - 사용자의 기기에 맞춰 적절한 크기의 이미지를 사동생성
