...
export default function MenuEditForm({menu}:Props){
    ...
    //     const response = await putMenu(menu.id, newMenus);
    //     if(response.status != 204){
    //         alert("수정 실패"); // 실패시 메시지 띠워주기
    //     }

    //     router.push('/menus'); // 메뉴로 넘겨주기
    //     router.refresh(); // 새로고침
    ...
    const handleSubmit = async (e:React.SubmitEvent) => {
        ...
        try{
            const response = await editMenu(menu);
            
            if(response.status != 204){
                alert("수정 실패"); // 실패시 메시지 띠워주기
            }
            router.push(`/menus/${menu.id}`); // 메뉴로 넘겨주기
            router.refresh(); // 새로고침
        }catch(err){
            setError('수정중 오류 발생')
        }finally{
