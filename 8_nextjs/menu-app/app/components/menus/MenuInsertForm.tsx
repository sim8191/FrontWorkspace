...
export default function MenuInsertForm(){
    // 겍체 바인딩
    const [menu, handleInputChange] = useInput<MenuCreate>(initMenu); // 내가 입력한 값 
    ...
    */
    ...
    const insertMenu = async(e:React.SubmitEvent) => {
        ...
        // e:React.SubmitEvent랑 e.preventDefault()는 한쌍으로 생각하면 좋음
        e.preventDefault(); // (이벤트를 발생시킬 때 페이지 새로고침을 방지 코드)
        setIsPending(true);
        const response = await createMenu(menu);
        if(response.status != 201){
            alert("등록 실패"); // 실패시 메시지 띠워주기
        }
        // response.headers.get
        const loc = response.headers['location'];

        router.push(loc ?? '/menus'); // 메뉴로 넘겨주기
        router.refresh(); // 새로고침
