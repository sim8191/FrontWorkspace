...
/*
    #1. Middleware
     ...
     - MextResponse : 로직 수행 후 다음단계로 넘길 때 사용하는 객체 
*/ // 이거는 프로젝트당 1개만 그리고 위치는 꼭 여위치(이거 파일 어디있는지 옆에서 봐)
export async function middleware(request:NextRequest){
    ...
    /*
        1. 엑세스 토큰은 만료되어 없으나, 리프레쉬 토큰만 남아 있는 경우
   ...
   if(!accessToken && refreshToken){
        ...
        if(refreshRes.ok){
            ...
            if(setCookie){
                ...
                const newAccessToken = setCookie.split(";").find(
                    c => c.trim().startsWith("accessToken=")
                )?.split("=")[1];
                const newRoles = setCookie.split(";").find(
                    c => c.trim().startsWith("userRoles=")
                )?.split("=")[1];
                if(newAccessToken){
                    ...
                    requestHeaders.set('x-new-access-token',newAccessToken);//x-new-access-token이거는 커스텀 헤더임
                ...
                if(newRoles){ // 권한정보 동기화
                    userRoles = decodeURIComponent(newRoles).split("|");
                }
            }else{
                // 리프레시 실패시 쿠키 삭제 후 로그인 페이지
    ...
    const protectedPathProfixes = ['/menus/', '/admin'];
    // 현재 사용자가 요청한 경로가 보호중인 페이지인지 확인
    const isProtectedPage 
    = protectedPathProfixes.some(prefix => pathname.startsWith(prefix));
    if(isProtectedPage && !refreshToken){ // 보호대상인데 인증되지 않은 경우
